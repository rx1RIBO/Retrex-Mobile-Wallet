package rext.org.rextwallet.rate.db;
...
import rext.org.rextwallet.contacts.AbstractSqliteDb;
...
public class RateDb extends AbstractSqliteDb<RextRate> implements RateDbDao<RextRate> {
    ...
    @Override
    protected ContentValues buildContent(RextRate obj) {
    ...
    @Override
    protected RextRate buildFrom(Cursor cursor) {
        ...
        return new RextRate(coin,value,timestap);
    }

    public RextRate getRate(String coin){
        return get(KEY_COIN,coin);
    }


    public void insertOrUpdateIfExist(RextRate rextRate) {
        if (getRate(rextRate.getCode())==null){
            insert(rextRate);
        }else {
            updateByKey(KEY_COIN,rextRate.getCode(),rextRate);
        }
    }
}
