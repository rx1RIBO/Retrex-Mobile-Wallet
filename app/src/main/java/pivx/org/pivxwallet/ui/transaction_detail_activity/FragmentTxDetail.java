...
import org.rextj.core.TransactionOutPoint;
import org.rextj.core.TransactionOutput;
import org.rextj.script.Script;
...
import static rext.org.rextwallet.ui.transaction_send_activity.custom.inputs.InputsActivity.INTENT_NO_TOTAL_AMOUNT;
...
public class FragmentTxDetail extends BaseFragment implements View.OnClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (intent != null){
            ...
            if (intent.hasExtra(IS_DETAIL)){
                ...
                transactionWrapper.setTransaction(new Transaction(rextModule.getConf().getNetworkParams(),intent.getByteArrayExtra(TX)));
        ...
        if (rextRate != null) {
            txt_amount_local.setText(
                    rextApplication.getCentralFormats().format(
                            new BigDecimal(transactionWrapper.getAmount().getValue() * rextRate.getRate().doubleValue()).movePointLeft(8)
                    )
                            + " " + rextRate.getCode()
            );
        } else {
        ...
        if (transactionWrapper.isStake()){
            ...
            try {
                ...
                for (TransactionInput input : transactionWrapper.getTransaction().getInputs()) {
                    ...
                    Coin unspentValue = rextModule.getUnspentValue(unspent.getHash(), (int) unspent.getIndex());
            ...
            if (transactionWrapper.getOutputLabels()!=null && transactionWrapper.getOutputLabels().containsKey(transactionOutput.getIndex())){
                ...
                if (addressLabel !=null) {
                    if (addressLabel.getName() != null) {
                        ...
                        label = transactionOutput.getScriptPubKey().getToAddress(rextModule.getConf().getNetworkParams(),true).toBase58();
                }else {
                    label = transactionOutput.getScriptPubKey().getToAddress(rextModule.getConf().getNetworkParams(),true).toBase58();
                }
            }else {
                ...
                if (script.isPayToScriptHash() || script.isSentToRawPubKey() || script.isSentToAddress()) {
                    label = script.getToAddress(rextModule.getConf().getNetworkParams(), true).toBase58();
                }else {
    ...
    private void setupOutputs(List<OutputUtil> list) {
        ...
        ListItemListeners<OutputUtil> listItemListener = new ListItemListeners<OutputUtil>() {
            ...
            public void onLongItemClickListener(OutputUtil data, int position) {
                if (rextModule.chechAddress(data.getLabel())) {
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.txt_inputs){
            try {
                Set<InputWrapper> set = rextModule.convertFrom(transactionWrapper.getTransaction().getInputs());
